...
void displayMainScreen() {
  ...
  // Farbe fuer Staedtezeiten: ROT bei LTE-Fehler, sonst GELB
  uint16_t cityColor = lteConnected ? ST7735_YELLOW : ST7735_RED;
  uint16_t timeColor = lteConnected ? ST7735_YELLOW : ST7735_YELLOW;

  // Erste Zeile: DXB, SIN, IAD
  tft.setFont();
  tft.setTextColor(cityColor);
  ...
  sprintf(anzeige, "%02i:%02i  %02i:%02i  %02i:%02i",
  ...
  // Dritte Zeile: Wechselkurse (nur anzeigen wenn gueltig)
  if (currenciesValid) {
    tft.setFont();

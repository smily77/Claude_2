...
void initializeTimezones() {
  const int DATABASE_SIZE = 86;  // Anzahl Einträge in AIRPORT_DATABASE

  for (int i = 0; i < 7; i++) {
    boolean found = false;

    // Durchsuche Datenbank nach Airport-Code
    for (int j = 0; j < DATABASE_SIZE; j++) {
      AirportTimezone apt;
      memcpy_P(&apt, &AIRPORT_DATABASE[j], sizeof(AirportTimezone));

      if (strcmp(AIRPORT_CODES[i], apt.code) == 0) {
        // Airport gefunden, kopiere Daten
        timezones[i].airCode = String(apt.code);
        timezones[i].stdOffset = apt.stdOffset;
        timezones[i].dstOffset = apt.dstOffset;
        timezones[i].dstType = apt.dstType;
        found = true;

        tft.print(apt.code);
        tft.print(": ");
        tft.print(apt.stdOffset / 3600.0);
        tft.println(" h");

        if (DEBUG) {
          Serial.print("Found: ");
          Serial.print(apt.code);
          Serial.print(" -> ");
          Serial.print(apt.stdOffset / 3600.0);
          Serial.println(" h");
        }
        break;
      }
...
time_t getNtpTime() {
  ...
  while (millis() - beginWait < 1500) {
    ...
    if (size >= NTP_PACKET_SIZE) {
      ...
      // Zeit für lokale Zeitzone (Zürich) berechnen
      tempTime = secsSince1900 - 2208988800UL;
      tempTime += getTimezoneOffset(0, tempTime); // Offset für Zürich (Zeitzone 0)
...
time_t getTimeForTimezone(int tzIndex, time_t localTime) {
  // localTime ist die Zeit in Zürich (Zeitzone 0)
  // Berechne UTC-Zeit
